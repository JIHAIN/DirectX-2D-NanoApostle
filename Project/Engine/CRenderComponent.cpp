...
void CRenderComponent::SetMaterial(wstring _MaterName)
{
	Ptr<CMaterial> Mrtl = CAssetMgr::GetInst()->FindAsset<CMaterial>(_MaterName);

	// 렌더컴포넌트는 원본재질만 세팅 받을 수 있다.	
	assert(!(nullptr != Mrtl && Mrtl->GetSharedMtrl().Get()));

	m_CurMtrl = m_SharedMtrl = Mrtl;
}
