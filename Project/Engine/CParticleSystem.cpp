...
// 텍스쳐랑 파티클 정보 세팅해주는 모드
CParticleSystem::CParticleSystem(tParticleModule _Module, Ptr<CTexture> _Tex)
	: CRenderComponent(COMPONENT_TYPE::PARTICLE_SYSTEM)
	, m_ParticleBuffer(nullptr)
	, m_SpawnCountBuffer(nullptr)
	, m_ModuleBuffer(nullptr)
...
void CParticleSystem::CaculateSpawnCount()
{
	...
	// 한번에 확 나오게 하는 버스트 모드가 켜져있으면 여기도 들어옴
