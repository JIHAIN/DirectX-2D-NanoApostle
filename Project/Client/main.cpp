...
{   
  ...
  // 레벨 로드 콜백 등록
    auto callback = [](const wstring& _LevelName) {
        if (_LevelName == L"Main")
            MainLevel::CreateMainLevel();
        else if (_LevelName == L"Boss")
            BossLevel::CreateBossLevel();
        else if (_LevelName == L"Trap")
            TrapLevel::CreateTrapLevel();
        };

    CLevelMgr::GetInst()->AddLevelLoadCallback(callback);
